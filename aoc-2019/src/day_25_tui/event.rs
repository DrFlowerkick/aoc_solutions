use super::{IntCodeTask, ShipRoom};
...
pub enum AppEvent {
    ...
    /// move left
    Left,
    /// previous item in room
    PreviousRoomItem,
    /// next item in room
    NexRoomItem,
    /// put room item into collection
    TakeRoomItem,
    /// previous collected item
    PreviousCollectedItem,
    /// next collected item
    NextCollectedItem,
...
impl EventHandler {
    ...
    pub fn new(mut int_code_task: IntCodeTask) -> Self {
        let (sender, receiver) = mpsc::unbounded_channel();
        let int_code_sender = sender.clone();
        tokio::spawn(async move { int_code_task.receive_messages(int_code_sender).await });
